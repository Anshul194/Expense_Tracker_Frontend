    ...
    return (
        <div className="max-w-7xl mx-auto space-y-12 pb-24">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pt-6">
                <div className="space-y-4">
                    ...
                    <div className="space-y-1">
                        <h1 className="text-6xl font-black tracking-tighter text-slate-950 dark:text-white leading-[1]">Overview.</h1>
                        <p className="text-slate-500 font-medium ml-1">Account synchronized for current billing cycle</p>
                    </div>
                </div>
                <div className="flex items-center gap-4">
                    <Button onClick={() => navigate('/expenses')} className="h-14 px-8 rounded-2xl bg-slate-950 hover:bg-slate-800 text-white font-black uppercase tracking-widest text-[10px] shadow-2xl transition-transform active:scale-95">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
                ...
                <Card className="lg:col-span-8 p-0 border-none bg-indigo-600 rounded-[2.5rem] overflow-hidden flex flex-col justify-between group">
                    ...
                    <div className="p-10 relative z-10 flex flex-col md:flex-row justify-between gap-10">
                        <div className="space-y-10">
                            <div>
                                <p className="text-white/60 text-[10px] font-black uppercase tracking-widest mb-1">Periodic Consumption</p>
                                <h2 className="text-6xl font-black text-white tracking-tighter leading-none">₹{summary?.currentMonthTotalSpending?.toLocaleString()}</h2>
                            ...
                            <div className="flex gap-10">
                                <div>
                                    ...
                                    <p className="text-white text-xl font-bold">₹{summary?.averageDailySpending?.toLocaleString()}</p>
                                </div>
                                <div className="w-px h-10 bg-white/10" />
                                <div>
                                    <p className="text-white/40 text-[9px] font-black uppercase tracking-widest mb-1">Transaction Index</p>
                                    <p className="text-white text-xl font-bold">{summary?.currentMonthExpenseCount}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-slate-950/20 backdrop-blur-md p-6 flex justify-between items-center px-10">
                ...
                <Card className="lg:col-span-4 p-8 bg-white dark:bg-slate-900 border-slate-200/60 flex flex-col justify-between h-full group cursor-pointer" onClick={() => navigate('/ai')}>
                    <div className="space-y-6">
                        <div className="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-500/10 flex items-center justify-center text-indigo-600 transition-transform group-hover:scale-110">
                            <Zap size={24} className="fill-indigo-600" />
                        </div>
                        <div>
                            <h3 className="text-3xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">Semantic <br />Audit.</h3>
                            <p className="text-slate-500 text-xs font-medium mt-4 leading-relaxed">Launch AI audit to discover latent spending vectors across your billing cycles.</p>
                        </div>
                    </div>
                    <div className="pt-10 flex justify-between items-center text-indigo-600 font-black text-[10px] uppercase tracking-[0.2em]">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-2 p-10 border-slate-200/60">
                    <div className="flex items-start justify-between mb-12">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-none mb-2">Spending Velocity</h3>
                            <p className="text-xs text-slate-500 font-medium">Daily transaction dynamics (Last 7 Sessions)</p>
                        </div>
                        <Badge variant="indigo" className="rounded-lg px-3 py-1 font-black text-[9px] uppercase tracking-widest opacity-60">Linear-Trend</Badge>
                    </div>
                ...
                <Card className="p-10 border-slate-200/60 flex flex-col justify-between">
                    <div className="mb-10">
                        <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter leading-none mb-2">Breakdown.</h3>
                        ...
                        {breakdown.slice(0, 5).map((item, index) => (
                            <div key={item.name} className="flex flex-col gap-2 group">
                                <div className="flex justify-between items-center text-xs font-black uppercase tracking-widest text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">
                                    ...
                                    <span className="text-[10px] text-slate-950 dark:text-white">₹{item.totalAmount?.toLocaleString()}</span>
                                </div>
                    ...
                    <button onClick={() => navigate('/categories')} className="mt-10 py-4 w-full rounded-2xl border border-slate-100 dark:border-slate-800 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
